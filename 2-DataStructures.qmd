...
```{r}
even_numbers_vector <- seq(from = 2, to = 18, by = 2)

matrix(even_numbers_vector, byrow = TRUE, nrow = 3)
```
...
CA_state <- gayguides$state=="CA"

seventies <- gayguides$Year>=1970 & gayguides$Year<=1979

CA_70s <- CA_state & seventies

head(CA_70s)

sub_CA <- gayguides[CA_70s,]

```

```{r}
gay_70s <- gayguides[gayguides$Year>=1970 & gayguides$Year<=1979,]

gay_CA_70s <- gay_70s[gay_70s$state=="CA",]
...
yr1975 <- gayguides[gayguides$Year==1975,]

girls <- grep("G", yr1975$amenityfeatures, ignore.case = TRUE)
...
B50 <-BostonWomenVoters[BostonWomenVoters$Age>= 50,]
BU50 <-BostonWomenVoters[BostonWomenVoters$Age<50,]
...
Bmain <- grep("G", BostonWomenVoters$Street.of.Residence.on.April.1, ignore.case = TRUE)
```
...
age_order <- order(BostonWomenVoters$Age, decreasing = TRUE)

by_age <- BostonWomenVoters[age_order,]
